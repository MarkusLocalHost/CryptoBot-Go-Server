...
type addUserToAdminsReq struct {
	UserID         string `json:"userID"`
	TelegramUserId string `bson:"telegramUserId"`
}

// AddUserToAdmins Create price observer
...
func (h *Handler) AddUserToAdmins(c *gin.Context) {
	var req addUserToAdminsReq
	...
	err = h.LogService.LogRequestFromSite(ctx, "/manager/utils/add_user_to_admin_group", req.UserID, req)
	if err != nil {
		log.Fatal(err)
	}

	// save observer to db
	err = h.ManagerService.AddUserToAdminGroup(ctx, int64(telegramUserId))
	if err != nil {
