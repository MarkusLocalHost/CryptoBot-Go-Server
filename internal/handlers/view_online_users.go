...
type viewOnlineUsersReq struct {
	UserID string `json:"userID"`
}

// ViewOnlineUsers Create price observer
...
func (h *Handler) ViewOnlineUsers(c *gin.Context) {
	var req viewOnlineUsersReq
	...
	err = h.LogService.LogRequestFromSite(ctx, "/manager/info/view_online_users", req.UserID, req)
	if err != nil {
		log.Fatal(err)
	}

	// save observer to db
	onlineUsersInfo, err := h.ManagerService.ViewOnlineUsersStats(ctx)
	if err != nil {
