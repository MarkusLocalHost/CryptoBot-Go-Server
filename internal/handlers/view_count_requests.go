...
type viewCountRequestsReq struct {
	UserID string `json:"userID"`
}

// ViewCountRequests Create price observer
...
func (h *Handler) ViewCountRequests(c *gin.Context) {
	var req viewCountRequestsReq
	...
	err = h.LogService.LogRequestFromSite(ctx, "/manager/info/view_count_requests", req.UserID, req)
	if err != nil {
		log.Fatal(err)
	}

	// save observer to db
	countRequestsInfo, err := h.ManagerService.ViewCountRequests(ctx)
	if err != nil {
